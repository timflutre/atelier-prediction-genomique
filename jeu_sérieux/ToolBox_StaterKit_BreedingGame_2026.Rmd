...
1. Se rendre sur le site [PlantBreedGame](http://147.99.23.29:8080)
...
```{r p2f}
setwd("~/Documents/Git/atelier-prediction-genomique/jeu_sérieux/data/")
...
```{r load}
pheno <- read.table(file="phenotypes.tsv",sep="\t", 
...
```{r}
Controls <- read.table(file="plant-material.tsv", sep="\t",header=TRUE)
(Controls <- c(Controls$Name))
```

Tous les génotypes contrôles ont été phénotypés pendant 4 années.

```{r load pheno control}
pheno_ctl <- read.table(file="phenotypes_controls.tsv",sep="\t", 
                    header=TRUE, fileEncoding = "UTF-8")
head(pheno_ctl)
```
...
```{r load geno data}
geno <- read.table(file="Result_genos_subset-initialColl-hd.txt.gz",
...
snp_map <- read.table(file="snp_coords_hd.txt.gz", sep="\t", header=TRUE)
head(snp_map)
dim(snp_map)
