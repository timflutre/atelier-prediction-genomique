...
Au cours d'un programme de sélection, génération après génération, l'augmentation de la valeur génotypique additive moyenne est communément appelée "progrès génétique".
...
Sous forme vectorielle, cette somme des carrés s'écrit comme une norme euclidienne ($|| \boldsymbol{\epsilon} ||^2$), aussi appelée "norme $L^2$".
Dans le cas de la régression d'arête, un terme de pénalité est ajouté à la vraisemblance, terme qui dépend aussi de la norme $L^2$ des effets pour la minimiser: $|| \boldsymbol{\epsilon} ||^2 + \lambda || \boldsymbol{\beta} ||^2$, où $\lambda$, paramètre à estimer, corresponds à la pénalité à appliquer aux effets.
...
Ceci permet de faire le lien entre l'apparentement attendu calculé à partir du pédigrée et l'apparentement réalisé estimé à partir des marqueurs.
En effet, en considérant les génotypes dans $X$ comme des variables aléatoires, et en posant certaines hypothèses, il s'avère que l'espérance $E(X X^T)$ est égale à $A_{\text{ped}} \times 2 \sum_p f_p (1 - f_p)$ à une constante près, où les $f_p$'s sont les fréquences alléliques des $P$ SNPs.
(La constante se retrouvera dans l'estimation de la moyenne globale $\mu$, et n'a donc pas grande importance.)
...
* et estimer l'héritabilité au sens strict, $\hat{h}^2 = \frac{\hat{\sigma}_a^2}{\hat{\sigma}_a^2 + \hat{\sigma}^2}$.
...
Une simulation moins simpliste nécessiterait un véritable scénario évolutif, par exemple en utilisant le coalescent avec recombinaison.
...
(h2 <- sigma.a2 / (sigma.a2 + sigma.epsilon2))
```
...
a <- mvrnorm(n=1, mu=rep(0, N), Sigma=sigma.a2 * A.mark)
y.poly <- matrix(1, nrow=N) * mu + a + epsilon
```
...
Dans une première étape, les composants de la variance, $\sigma_u^2$ et $\sigma^2$, sont estimés, mais sans inclure l'information des génotypes.
Dans une seconde étape, les effets alléliques sont estimés SNP par SNP et l'hypothèse nulle $\mathcal{H}_0:"\beta_p = 0"$ est testée pour chacun d'eux, en utilisant les estimations des composants de la variance de la première étape.
...
res.mono.gwas$vc <- estVC(y=y.mono, v=list(AA=A.mark, DD=NULL, HH=NULL, AD=NULL,
                                           MH=NULL, EE=diag(N)))
res.mono.gwas$scan <- scanOne(y=y.mono, gdat=X[,subset.snps],
...
res.poly.gwas$vc <- estVC(y=y.poly, v=list(AA=A.mark, DD=NULL, HH=NULL, AD=NULL,
                                           MH=NULL, EE=diag(N)))
res.poly.gwas$scan <- scanOne(y=y.poly, gdat=X[,subset.snps],
