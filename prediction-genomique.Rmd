...
Le modèle de régression d'arête \eqref{eqn:lmmRR} s'avère être équivalent au modèle suivant:
\begin{align}
...
Notez qu'on aurait aussi pu simuler les valeurs génotypiques additives via $\boldsymbol{u} \sim \mathcal{N}_N(\boldsymbol{0}, \sigma_u^2 A_{\text{mark}})$.
...
hist(y.mono, breaks="FD", las=1, col="grey", border="white",
     main="Caractère monogénique", xlab="Phénotypes", ylab="Nombre d'individus")
hist(y.poly, breaks="FD", las=1, col="grey", border="white",
     main="Caractère polygénique", xlab="Phénotypes", ylab="Nombre d'individus")
```
...
La manière habituelle de regarder les résultats du modèle d'inférence SNP à SNP \eqref{eqn:lmmGwas} est de tracer un *Manhattan plot* (regardez la fonction \verb+manhattan+ du paquet [qqman](https://cran.r-project.org/web/packages/qqman/)).
...
plot(beta.poly, res.poly.ridge$u, las=1, asp=1,
     xlab="Vrais effets alléliques additifs",
     ylab="Effets alléliques additifs estimés",
...
Notez que je parle parfois de valeurs génotypiques additives "estimées" et parfois de valeurs génotypiques additives "prédites".
Dans la littérature scientifique utilisant l'interprétation fréquentiste des probabilités, on parle de valeurs génotypiques additives "prédites".
Les inconnues $\boldsymbol{u}$ sont les *breeding values* et les résultats $\hat{\boldsymbol{u}}$ sont les *Best Linear Unbiased Predictions* (BLUPs) des *breeding values*.
De manière générale, dans cette littérature, les effets "fixes" sont "estimés" et les effets "aléatoires" sont "prédits".
