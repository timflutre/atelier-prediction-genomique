...
Dans le cas de la régression d'arête, un terme de pénalité est ajouté à la vraisemblance, terme qui dépend aussi de la norme $L^2$ des effets pour la minimiser: $|| \boldsymbol{\epsilon} ||^2 + \lambda || \boldsymbol{\beta} ||^2$, où $\lambda$, paramètre à estimer, correspond à la pénalité à appliquer aux effets.
...
En effet, en considérant les génotypes dans $X$ comme des variables aléatoires, et en posant certaines hypothèses (voir Habier et coll., 2007), il s'avère que l'espérance $E(X X^T)$ est égale à $A_{\text{ped}} \times 2 \sum_p f_p (1 - f_p)$ à une constante près, où les $f_p$'s sont les fréquences alléliques des $P$ SNPs.
