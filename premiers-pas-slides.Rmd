...
author: "Vincent Segura (INRAE) <br><br> d'après ['premiers-pas.Rmd'](https://github.com/timflutre/atelier-prediction-genomique/blob/master/prediction-genomique.Rmd) de Timothée Flutre (INRAE) <br>"
...
<!-- ## A propos de modélisation statistique {.build} -->
...
<!-- <div> -->
<!-- - Validité du modèle : vérifier que les données à analyser correspondent bien à la question à laquelle on veut répondre ([Gelman & Hill, 2006](http://www.worldcat.org/isbn/0521867061)) -->
<!-- </div> -->

<!-- <div> -->
<!-- - Distinguer le "monde réel" (données) du "monde théorique" (modèles) : <br> -->
<!-- "When we use a statistical model to make a statistical inference, we implicitly assert that the variation exhibited by data is captured reasonably well by the statistical model, so that the theoretical world corresponds reasonably well to the real world." ([Kass, 2011](http://dx.doi.org/10.1214/10-sts337)) -->

<!-- - Ne pas confondre les données avec des variables aléatoires, même si on fait souvent le raccourci -->
<!-- </div> -->
...
<!-- On peut écrire la vraisemblance sous forme plus explicite comme une fonction des paramètres $\Theta = \{ \mu, \beta, \sigma \}$: -->
<!-- $$ -->
<!-- \begin{align} -->
...
<!-- - On travaille avec la log vraisemblance et on utilise les [règles d'analyse](https://en.wikipedia.org/wiki/Differentiation_rules) : -->
...
<!-- avec $\bar{x} = \frac{1}{n} \sum_i x_i$ et $\bar{y} = \frac{1}{n} \sum_i y_i$. -->
<!-- </div> -->

<!-- ## Evaluation de la qualité du modèle -->

<!-- - Dans le cas de la régression linéaire simple on peu utiliser le coefficient de détermination ($R^2$), interprété comme la proportion de la variance phénotypique expliquée par les prédicteurs : -->
<!-- $$ -->
<!-- \begin{align} -->
<!-- R^2 = \frac{Var(\hat{\mu} + \hat{\beta} \boldsymbol{x})}{Var(\boldsymbol{y})} = \frac{\hat{\beta}^2 \; Var(\boldsymbol{x})}{\hat{\beta}^2 \; Var(\boldsymbol{x}) + \hat{\sigma}^2} \\ -->
<!-- \end{align} -->
<!-- $$ -->
<!-- $$ -->
<!-- \begin{align} -->
<!--  \Leftrightarrow \; \; R^2 = \frac{\sum_i ((\hat{\mu} + \hat{\beta} x_i) - \bar{y})^2}{\sum_i (y_i - \bar{y})^2} -->
<!-- \end{align} -->
<!-- $$ -->
...
- On simule des vecteurs aléatoires avec la fonction **mvrnorm** du package *MASS*
