...
Concentrons-nous sur un seul caractère d'une espèce donnée, et imaginons une population de $I$ génotypes, chacun ayant une valeur génotypique $g_i$, avec $i \in \{1,\ldots,I\}$.
Dans la théorie classique de la génétique quantitative, la valeur génotypique se décompose en valeurs additive ($a_i$), de dominance ($d_i$) et d'épistasie ($\zeta_i$), $g_i = a_i + d_i + \zeta_i$, de telle sorte que $a_i$ corresponde à la part héritable, c'est-à-dire transmise à la descendance, donc particulièrement d'intérêt en sélection, d'où le fait qu'on l'appelle aussi \textit{breeding value}.
...
De manière similaire pour les deux autres.
...
Or les valeurs génotypiques ne sont pas observables, il faut donc les estimer à partir d'un échantillon de $N$ valeurs phénotypiques, notées $\{y_n\}_{1 \le n \le N}$, avec $\text{E}[y_n] = \mu_0$ et $\text{Var}[y_n] = \sigma_p^2$.
...
\[
\forall n, \, y_n | \mu_0, \sigma_p^2 \, \sim \, \mathcal{N}(\mu_0, \sigma_p^2)
\]
...
\[
\forall i,j, \, y_{ij} = \mu + g_i + \epsilon_{ij} \text{ avec } \epsilon_{ij} \sim \mathcal{N}(0, \sigma^2) \; \Leftrightarrow \; \forall i,j, \; y_{ij} | \mu, g_i, \sigma^2 \, \sim \, \mathcal{N}(\mu + g_i, \sigma^2)
\]
...
Les composantes de la variance sont également utilisées pour calculer la corrélation entre les valeurs génotypiques et les valeurs phénotypiques:

\[
\frac{\text{Cov}[g, y]}{\sigma_g^2 \; \sigma_p^2} = \frac{\sigma_g^2 + \text{Cov}[g, \epsilon]}{\sigma_g^2 \; \sigma_p^2}
\]

Dans le cas où il n'y a pas de covariance entre génotypes et environnement, on appelle **héritabilité au sens large** le carré de cette corrélation, noté:
...
Pour cela, notons $y_{\text{mère}}$ (resp. $y_{\text{père}}$) une valeur phénotypique de la mère (resp. du père) avec $y$ leur moyenne, $y = \frac{y_{\text{mère}} + y_{\text{père}}}{2}$, et $y_e$ une valeur phénotypique de leur enfant.
...
où $h^2$ est appelée **héritabilité au sens strict**.
...
S'il n'y a pas de sélection, c'est-à-dire que tout génotype peut être parent, les moyenne et variance phénotypique à la génération des enfants sont égales à celles des parents, $\mu_0$ et $\sigma_p^2$.
Comme la pente de la droite, $\beta_{\text{enfants,parents}}$, correspond à $h^2$, on peut en déduire la covariance entre valeurs phénotypiques des parents et enfants, et ainsi simuler toutes les valeurs phénotypiques via une loi Normale bivariée.
...
```
```{r, echo=FALSE}
tmp <- hist(x=y, breaks="FD", col="grey", border="white", las=1,
...
Dans le cas où les valeurs utilisés pour la sélection (ici les valeurs génotypiques $\{g_i\}_{1 \le i \le I}$) suivent une loi Normale, un autre avantage de l'intensité $i$ est de permettre d'approximer sa relation avec le **taux de sélection** $\alpha$:
...
```{r}
alpha <- c(0.01, 0.05, 0.1, 0.15, 0.2, 0.5) # from 1% to 20%
...
text(x=z[idx]*1.4, y=phi.z[idx]*1.4,
     labels=paste0("alpha=", format(100*alpha[idx], digits=2), "%",
                   "; i=", format(i[idx], digits=3)))
...
L'héritabilité au sens strict, $h^2$, étant inférieure à 1, la droite de régression des enfants sur les parents n'est donc pas aussi pentue que la droite identité.
C'est cette observation qui a amené Galton a parlé de "régression".

Or, si l'on compare la moyenne des parents sélectionnés, $\mu^{(s)}$, avec celle de leurs enfants, notée $\mu_1$, on voit une augmentation, ce qui amène à définir la **réponse à la sélection** (aussi appelé **gain génétique**):
...
abline(v=mu.s, lty=3, lwd=2)
text(x=mu.s+0.6, y=34.4, labels=expression(mu^"(s)"), cex=1.5)
...
On voit que le changement, dû à la sélection, de moyenne phénotypique au sein des parents ($S$) induit bien un changement de moyenne phénotypique au sein de leurs enfants:
...
On peut aussi voir $h$ comme la corrélation entre les valeurs phénotypiques $\{y_n\}$ et les valeurs génotypiques additives, $\{a_i\}$, d'où le fait que $h$ soit aussi noté $r$ pour indiquer que ça mesure la précision (\textit{accuracy}) avec laquelle les valeurs phénotypiques permettent d'approximer les valeurs génotypiques additives:
...
A budget constant, il est donc nécessaire d'optimiser les paramètres d'un programme de sélection, et de concevoir une stratégie.

Si on sélectionne très fortement dès la première génération en ne permetant qu'à un tout petit nombre de génotypes de se reproduire, on va certes augmenter fortement la moyenne de la génération suivante, mais on va aussi diminuer drastiquement la variance génétique.
Or, en pratique, il est très fréquent qu'il faille explorer un très grand nombre de combinaisons (par croisement) avant d'obtenir des génotypes performants.
