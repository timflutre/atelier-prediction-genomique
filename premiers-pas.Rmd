...
Le format Rmd permet également d'utiliser le language LaTeX pour écrire des équations. Pour en savoir plus, reportez-vous au [livre en ligne](https://fr.wikibooks.org/wiki/LaTeX).
...
\begin{align*}
\mathcal{L}(\Theta | \mathcal{D} ) &= p(\boldsymbol{y} | \boldsymbol{x}, \Theta) = p(y_1,\ldots,y_n | x_1,\ldots,x_n, \mu, \beta, \sigma) \\
...
```{r look_geno_pheno}
boxplot(phenotypes ~ genotypes, varwidth=TRUE, notch=TRUE,
        xlab="Genotypes", ylab="Phenotypes", las=1)
```

Remarquez l'importance de faire des graphiques les plus clairs et intelligibles possible.
Ce ne sont souvent que les graphiques que l'on montre à la place des tableaus de résultats, ceux-ci étant connus pour être plus difficiles à lire ([Gelman *et al.*, 2002](http://dx.doi.org/10.1198/000313002317572790)).
...
Du matériel pédagogique sur le modèle linéaire est aussi disponible en accès libre sur le site internet de la Société Française de Statistique ([ici](http://www.sfds.asso.fr/370-Ressources_pour_lenseignement_par_filieres)).
...
On peut facilement vérifier que cette valeur renvoyée par la fonction \verb+lm+ correspond à la formule écrite ci-dessus:
```{r calc_adjust}
...
Nous pouvons aussi calculer l'erreur quadratique moyenne avec les phénotypes déjà observés:
```{r calc_rmse_y}
y <- phenotypes
...
Mais, de façon plus intéressante, nous voudrions évaluer les prédictions de nouveaux phénotypes.
Pour cela, commençons par simuler de nouvelles données, toujours avec les mêmes "vraies" valeurs des paramètres:
```{r simul_new_data}
set.seed(1944) # année de découverte de l'ADN comme support des gènes
n.tilde <- 500
x.new <- sample(x=c(0,1,2), size=n.tilde, replace=TRUE, prob=calcGenoFreq(f))
y.new <- mu + beta * x.new + rnorm(n=n.tilde, mean=0, sd=sigma)
```

Puis utilisons nos estimations obtenues précédemment pour prédire ces nouveaux phénotypes à partir de ces nouveaux génotypes:
```{r predict_ytilde}
y.tilde <- (coefficients(fit)[1] + coefficients(fit)[2] * x.new)
```

Enfin, calculons l'erreur quadratique de prédiction:
```{r rmspe}
errors <- y.new - y.tilde
...
```{r ynew_vs_ytilde}
plot(x=y.new, y=y.tilde, xlab="New phenotypes", ylab="Predicted phenotypes", las=1)
...
\[
\boldsymbol{y} | W, X, \boldsymbol{\alpha}, \boldsymbol{\beta}, \sigma \; \sim \mathcal{N}_n(W \boldsymbol{\alpha} + X \boldsymbol{\beta}, \Sigma) \text{ avec } \Sigma = \sigma^2 I_n
\]

où $W$ est la matrice qui contient la colonne de 1 ainsi que potentiellement d'autres colonnes (si besoin est), $\boldsymbol{\alpha}$ est le vecteur contenant la moyenne globale $\mu$ ainsi que d'autres effets (si besoin est) considérés comme "fixes", $X$ est une matrice potentiellement à plusieurs colonnes, ici contenant les génotypes, $\boldsymbol{\beta}$ est un vecteur contenant des effets associés aux colonnes de $X$ et considérés comme "aléatoires", et $\Sigma$ est la matrice de variance-covariance.

Sous cette forme, ne devient-il pas "facile" d'inclure dans notre modèle initial, la régression linéaire simple, certains élements proposés en perspective?
