...
Nous retrouvons les valeurs utilisées pour simuler les données.
C'est là l'intérêt de la simulation par rapport à des vraies données: on connaît les valeurs que l'on est censé retrouver.

Nous pouvons aussi extraire une à une de l'objet renvoyé par la fonction \verb+lm()+ les quantités qui nous intéressent et vérifier avec les formules:
```{r get_results}
c(coefficients(fit)[1], coefficients(fit)[2], summary(fit)$sigma)
c(mu, beta, sigma)
(se.mu.hat <- summary(fit)$coef[1,2])
(se.beta.hat <- summary(fit)$coef[2,2])
