...
Tout naturellement, la méthode du *maximum de vraisemblance* cherche donc à identifier la valeur du paramètre, notée $\hat{\theta}$ par convention, qui maximise la vraisemblance:
...
En effet, certaines personnes décrivent les statistiques comme étant la *science de l'incertitude* ([Lindley, 2000](http://dx.doi.org/10.1111/1467-9884.00238)).
Or pour l'instant, nous n'avons parlé que de la façon d'obtenir *une* valeur par paramètre, celle qui maximise la vraisemblance des données.
Il est donc primordial ensuite de *quantifier l'incertitude* que nous avons concernant le fait que cette valeur soit "raisonnable".
C'est sur ce point que différentes approches diffèrent (*fréquentiste*, *bayésienne*).
Mais, quelle que soit l'approche, on peut s'attendre au fait que, plus le nombre d'échantillons augmente (s'ils sont "représentatifs), plus l'incertitude sur la valeur du paramètre diminue.
...
Comme il y a de la variation, on choisit d'introduire une *variable aléatoire* $Y$ correspondant à la quantité d'intérêt, et dénotons par $y_1$, $y_2$ et $y_3$ les trois observations, vues comme des réalisations de la variable aléatoire $Y$.
...
Etant donné les caractéristiques du phénomène ("somme d'une très grande quantité de facteurs indépendants, chacun ayant un faible impact"), il est raisonnable de supposer que la variable $Y$ suit une *loi Normale* (*c.f.* théorème central limite).
...
La moyenne $\mu$ peut prendre toutes les valeurs entre $-\infty$ et $+\infty$, et la variance $\sigma^2$ n'a pour seul restriction que d'être positive.
...
Parmi toutes les valeurs possibles des paramètres, le défi consiste donc à trouver celles pour lesquelles la loi Normale est une bonne description du mécanisme qui a généré nos données.
...
Pour rendre les choses plus facile, supposons que l'on sache déjà que la variance vaut 1.
Il ne nous reste plus qu'à trouver la moyenne.
...
D'après le graphique, la densité de probabilité $p(y_1 | \mu=5, \sigma=1)$ est strictement supérieur à $p(y_1 | \mu=2, \sigma=1)$.
...
Comme les deux densité ont la même valeur pour $\sigma$, la différence vient bien du terme $(y - \mu)^2$ dans l'exponentielle, qui représente l'éacrt à la moyenne.
...
<!--
Comme nous avons trois observations au total, nous pouvons faire de même:

```{r ex_lik_y, echo=FALSE}
x <- seq(from=-3, to=10, length.out=500)
plot(x=x, y=dnorm(x=x, mean=2, sd=1), type="l", col="blue",
     main="",
     xlab="quantiles", ylab="densité de probabilité")
...
legend("topright", legend=c("mu=2", "mu=5"), col=c("blue", "red"), lty=1, bty="n")
for(i in 1:length(y)){
  segments(x0=y[i], y0=0, x1=y[i], y1=dnorm(x=y[i], mean=2, sd=1))
  segments(x0=y[i], y0=0, x1=y[i], y1=dnorm(x=y[i], mean=5, sd=1))
  segments(x0=y[i], y0=dnorm(x=y[i], mean=2, sd=1),
           x1=-5, y1=dnorm(x=y[i], mean=2, sd=1), col="blue")
  segments(x0=y[i], y0=dnorm(x=y[i], mean=5, sd=1),
           x1=-5, y1=dnorm(x=y[i], mean=5, sd=1), col="red")
}
```

Ceci semble nous conforter dans notre choix de $\mu=5$.
-->
...
Dans le cas de la loi Normale, il existe déjà dans R des fonctions implémentant la densité de probablité, ce qui nous permet de vérifier que nous n'avons pas fait d'erreur:
```{r check_compute_lik}
...
La loi Normale ci-dessus est utilisée sous sa forme *univariée*, c'est-à-dire qu'on s'en sert pour tirer aléatoirement *une* réalisation de la variable aléatoire d'intérêt.

Or la même loi existe aussi sous forme *multivariée*, ce qui permet, à chaque fois qu'on "tire" dedans, d'obtenir *plusieurs* réalisations de la variable aléatoire d'intérêt, celles-ci étant arrangées dans un vecteur.
...
Sous cette forme, ne devient-il pas "facile" d'inclure dans le modèle les élements proposés en perspective ci-dessus?
