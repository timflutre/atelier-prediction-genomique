...
<div style="position: relative">
où $\sigma_a^2$ est estimé et $A$ est la matrice des relations génétique additives (**matrice d'apparentement**, *kinship*) calculée à partir de l'arbre généalogique (pédigrée) des individus
</div>
...
<div style="position: relative">
où la fonction $g$ correspond à l'**architecture génétique** du caractère d'intérêt, détaillée ci-après
</div>
...
- On se limite au cas simple d'architectures génétiques additives et on considére principalement deux cas extrêmes :

    1) **Caractère monogénique** déterminé un seul SNP a un effet non-nul, par exemple un SNP non-synonyme dans le seul gène causal
...
- Sous R, en utilisant la fonction `mvrnorm` du package [MASS](https://cran.r-project.org/web/packages/MASS/), cela aurait donné:
