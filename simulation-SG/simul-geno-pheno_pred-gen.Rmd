...
### Validation croisée bis avec les vraies valeurs génétiques

```{r}
out.cv <- matrix(nrow=folds$K, ncol=folds$R,NA)
...
for(r in 1:folds$R){
  for(k in 1:folds$K){
    ...
    fit <- rrBLUP::mixed.solve(y=y[in.train,],Z=X[in.train,])
    pred <-  X[in.train,] %*% as.matrix(fit$u) 
    out.cv[k,r] <- cor(pred,a[in.train,])
  }
}
```
